...
import time
import json 
import re # Импортируем регулярные выражения для очистки
...
def submit_donation():
    ...
    try:
        ...
        # 2. Агрессивная очистка строки для обработки региональных форматов (1 000,50 ₸)
        
        # Заменяем запятую на точку, если она используется как десятичный разделитель
        cleaned_amount = str(raw_amount).replace(',', '.')
        
        # Удаляем все, что не является цифрой или точкой (пробелы, символы валюты и т.д.)
        # Оставляем только первый найденный десятичный разделитель (точку)
        cleaned_amount = re.sub(r'[^\d.]', '', cleaned_amount)
        
        # Обрабатываем случай, когда остались две точки (например, 1.000.00 -> 1000.00)
        # Если есть несколько точек, удаляем все, кроме последней (или первой, в зависимости от формата)
        # Для простоты, удаляем все, кроме цифр и первого символа точки/запятой
        
        # Более простой подход: удаляем все нецифровые символы, кроме точки
        cleaned_amount = re.sub(r'[^\d.]', '', cleaned_amount)
