...
def submit_donation():
    ...
    try:
        ...
        # 2. Агрессивная очистка строки: удаляем все, кроме цифр.
        
        # Заменяем все запятые на точки (если они используются как десятичный разделитель).
        cleaned_amount = str(raw_amount).replace(',', '.')
        
        # Используем регулярное выражение для удаления всех символов, кроме цифр и точки.
        cleaned_amount = re.sub(r'[^\d.]', '', cleaned_amount)
        
        # Если в строке осталась более одной точки (например, 1.000.00), удаляем все, кроме последней.
        parts = cleaned_amount.split('.')
        if len(parts) > 1:
            # Склеиваем целую часть и оставляем только последнюю часть (дробную)
            cleaned_amount = "".join(parts[:-1]) + "." + parts[-1]
        elif len(parts) == 1:
            cleaned_amount = parts[0]

        # Если строка пуста после очистки, это неверный формат
        if not cleaned_amount:
            print(f"Критическая ошибка: Сумма '{raw_amount}' очищена до пустой строки.")
